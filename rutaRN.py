...
def generar_movimientos(tablero, secuencia, posicion_inicial=5):
    movimientos = []
...
def guardar_rutas(movimientos):
    with open("Rutas/todas_las_rutasRN.txt", "w") as todas, open("Rutas/rutas_ganadorasRN.txt", "w") as ganadoras, open("Rutas/rutas_perdedorasRN.txt", "w") as perdedoras:
...
if len(sys.argv) < 2:
    print("Uso: python nombre_del_script.py <secuencia>")
    sys.exit(1)

# Tomar la secuencia de colores del argumento de la línea de comandos
...
if movimientos_posibles:
    print("Calculando Ruta Rey Negro")
